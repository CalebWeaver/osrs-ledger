import { Method, MethodEvaluation, PriceMap, Requirement, Skill } from './types';

// Simple name mapping for MVP (In a real app, this would come from an item DB)
const ITEM_NAMES: Record<number, string> = {
  // Runes
  555: 'Water rune',
  558: 'Mind rune',
  561: 'Nature rune',
  563: 'Law rune',
  564: 'Cosmic rune',
  565: 'Blood rune',
  566: 'Soul rune',
  9075: 'Astral rune',
  // Materials
  1761: 'Soft clay',
  2355: 'Silver bar',
  13446: 'Dark essence block',
  1601: 'Diamond',
  1603: 'Ruby',
  1605: 'Emerald',
  1607: 'Sapphire',
  1615: 'Dragonstone',
  2357: 'Gold bar',
  6573: 'Onyx',
  // Moulds
  1592: 'Ring mould',
  1594: 'Unholy mould',
  1595: 'Amulet mould',
  1597: 'Necklace mould',
  1599: 'Holy mould',
  2976: 'Sickle mould',
  5523: 'Tiara mould',
  9434: 'Bolt mould',
  11065: 'Bracelet mould',
  // Rings
  1635: 'Gold ring',
  1637: 'Sapphire ring',
  1639: 'Emerald ring',
  1641: 'Ruby ring',
  1643: 'Diamond ring',
  1645: 'Dragonstone ring',
  6575: 'Onyx ring',
  // Necklaces
  1654: 'Gold necklace',
  1656: 'Sapphire necklace',
  1658: 'Emerald necklace',
  1660: 'Ruby necklace',
  1662: 'Diamond necklace',
  1664: 'Dragon necklace',
  6577: 'Onyx necklace',
  // Bracelets
  11069: 'Gold bracelet',
  11072: 'Sapphire bracelet',
  11076: 'Emerald bracelet',
  11085: 'Ruby bracelet',
  11092: 'Diamond bracelet',
  11115: 'Dragonstone bracelet',
  11130: 'Onyx bracelet',
  // Amulets (unstrung)
  1673: 'Gold amulet (u)',
  1675: 'Sapphire amulet (u)',
  1677: 'Emerald amulet (u)',
  1679: 'Ruby amulet (u)',
  1681: 'Diamond amulet (u)',
  1683: 'Dragonstone amulet (u)',
  6579: 'Onyx amulet (u)',
  // Runes (additional)
  554: 'Fire rune',
  556: 'Air rune',
  557: 'Earth rune',
  // Strung amulets (enchant inputs)
  1694: 'Sapphire amulet',
  1696: 'Emerald amulet',
  1698: 'Ruby amulet',
  1700: 'Diamond amulet',
  1702: 'Dragonstone amulet',
  6581: 'Onyx amulet',
  // Enchanted jewelry
  1712: 'Amulet of glory(4)',
  1725: 'Amulet of strength',
  1727: 'Amulet of magic',
  1729: 'Amulet of defence',
  1731: 'Amulet of power',
  2550: 'Ring of recoil',
  2552: 'Ring of duelling(8)',
  2568: 'Ring of forging',
  2570: 'Ring of life',
  2572: 'Ring of wealth',
  3853: 'Games necklace (8)',
  5521: 'Binding necklace',
  6583: 'Ring of stone',
  6585: 'Amulet of fury',
  11074: 'Bracelet of clay',
  11079: 'Castle wars bracelet(3)',
  11088: 'Inoculation bracelet',
  11090: 'Phoenix necklace',
  11095: 'Abyssal bracelet(5)',
  11105: 'Skills necklace(4)',
  11118: 'Combat bracelet(4)',
  11128: 'Berserker necklace',
  11133: 'Regen bracelet',
  11194: 'Digsite pendant(5)',
  // Standard teleport tablets
  8007: 'Varrock teleport (tablet)',
  8008: 'Lumbridge teleport (tablet)',
  8009: 'Falador teleport (tablet)',
  8010: 'Camelot teleport (tablet)',
  8011: 'Ardougne teleport (tablet)',
  8012: 'Watchtower teleport (tablet)',
  8013: 'Teleport to house (tablet)',
  28790: 'Kourend castle teleport (tablet)',
  28824: 'Civitas illa fortis teleport (tablet)',
  31441: 'Summon boat (tablet)',
  31443: 'Teleport to boat (tablet)',
  // Ancient teleport tablets
  12775: 'Annakarl teleport (tablet)',
  12776: 'Carrallanger teleport (tablet)',
  12777: 'Dareeyak teleport (tablet)',
  12778: 'Ghorrock teleport (tablet)',
  12779: 'Kharyrll teleport (tablet)',
  12780: 'Lassar teleport (tablet)',
  12781: 'Paddewwa teleport (tablet)',
  12782: 'Senntisten teleport (tablet)',
  // Lunar teleport tablets
  24949: 'Moonclan teleport (tablet)',
  24951: 'Ourania teleport (tablet)',
  24953: 'Waterbirth teleport (tablet)',
  24955: 'Barbarian teleport (tablet)',
  24957: 'Khazard teleport (tablet)',
  24959: 'Fishing guild teleport (tablet)',
  24961: 'Catherby teleport (tablet)',
  24963: 'Ice plateau teleport (tablet)',
  // Arceuus teleport tablets
  19613: 'Arceuus library teleport (tablet)',
  19615: 'Draynor manor teleport (tablet)',
  19617: 'Mind altar teleport (tablet)',
  19619: 'Salve graveyard teleport (tablet)',
  19621: "Fenkenstrain's castle teleport (tablet)",
  19623: 'West ardougne teleport (tablet)',
  19625: 'Harmony island teleport (tablet)',
  19627: 'Cemetery teleport (tablet)',
  19629: 'Barrows teleport (tablet)',
  19631: 'Ape atoll teleport (tablet)',
  22949: 'Battlefront teleport (tablet)',
  // Gems (semiprecious)
  1609: 'Opal',
  1611: 'Jade',
  1613: 'Red topaz',
  // Uncut gems (semiprecious)
  1625: 'Uncut opal',
  1627: 'Uncut jade',
  1629: 'Uncut red topaz',
  // Uncut gems (precious)
  1617: 'Uncut diamond',
  1619: 'Uncut ruby',
  1621: 'Uncut emerald',
  1623: 'Uncut sapphire',
  1631: 'Uncut dragonstone',
  6571: 'Uncut onyx',
  // Strung amulets (semiprecious, enchant inputs)
  21098: 'Opal amulet',
  21101: 'Jade amulet',
  21104: 'Topaz amulet',
  // Silver gem jewelry
  21081: 'Opal ring',
  21084: 'Jade ring',
  21087: 'Topaz ring',
  21153: 'Opal necklace',
  21156: 'Jade necklace',
  21159: 'Topaz necklace',
  21117: 'Opal bracelet',
  21120: 'Jade bracelet',
  21123: 'Topaz bracelet',
  21090: 'Opal amulet (u)',
  21093: 'Jade amulet (u)',
  21096: 'Topaz amulet (u)',
  // Enchanted semiprecious jewelry
  21160: 'Ring of pursuit',
  21143: 'Dodgy necklace',
  21107: 'Amulet of bounty',
  21126: 'Expeditious bracelet',
  21162: 'Ring of returning(5)',
  21146: 'Necklace of passage(5)',
  21110: 'Amulet of chemistry',
  21129: 'Flamtaer bracelet',
  21165: "Efaritay's aid",
  21169: 'Necklace of faith',
  21113: 'Burning amulet(5)',
  21132: 'Bracelet of slaughter',
  // Silver items
  1714: 'Unstrung symbol',
  1720: 'Unstrung emblem',
  2961: 'Silver sickle',
  5525: 'Tiara',
  9382: 'Silver bolts (unf)',
  // Orbs & Battlestaves
  569: 'Fire orb',
  571: 'Water orb',
  573: 'Air orb',
  575: 'Earth orb',
  1391: 'Battlestaff',
  1393: 'Fire battlestaff',
  1395: 'Water battlestaff',
  1397: 'Air battlestaff',
  1399: 'Earth battlestaff',
  // Bolt tips
  45: 'Opal bolt tips',
  46: 'Pearl bolt tips',
  9188: 'Topaz bolt tips',
  9189: 'Sapphire bolt tips',
  9190: 'Emerald bolt tips',
  9191: 'Ruby bolt tips',
  9192: 'Diamond bolt tips',
  9193: 'Dragonstone bolt tips',
  9194: 'Onyx bolt tips',
  // Base bolts
  877: 'Bronze bolts',
  9140: 'Iron bolts',
  9141: 'Steel bolts',
  9142: 'Mithril bolts',
  9143: 'Adamant bolts',
  9144: 'Runite bolts',
  21905: 'Dragon bolts',
  // Gem-tipped bolts
  879: 'Opal bolts',
  880: 'Pearl bolts',
  9336: 'Topaz bolts',
  9337: 'Sapphire bolts',
  9338: 'Emerald bolts',
  9339: 'Ruby bolts',
  9340: 'Diamond bolts',
  9341: 'Dragonstone bolts',
  9342: 'Onyx bolts',
  // Dragon gem-tipped bolts
  21955: 'Opal dragon bolts',
  21959: 'Pearl dragon bolts',
  21961: 'Topaz dragon bolts',
  21963: 'Sapphire dragon bolts',
  21965: 'Emerald dragon bolts',
  21967: 'Ruby dragon bolts',
  21969: 'Diamond dragon bolts',
  21971: 'Dragonstone dragon bolts',
  21973: 'Onyx dragon bolts',
  // Fletching - Darts
  314: 'Feather',
  806: 'Bronze dart',
  807: 'Iron dart',
  808: 'Steel dart',
  809: 'Mithril dart',
  810: 'Adamant dart',
  811: 'Rune dart',
  819: 'Bronze dart tip',
  820: 'Iron dart tip',
  821: 'Steel dart tip',
  822: 'Mithril dart tip',
  823: 'Adamant dart tip',
  824: 'Rune dart tip',
  11230: 'Dragon dart',
  11232: 'Dragon dart tip',
  // Fletching - Arrows
  39: 'Bronze arrowheads',
  40: 'Iron arrowheads',
  41: 'Steel arrowheads',
  42: 'Mithril arrowheads',
  43: 'Adamant arrowheads',
  44: 'Rune arrowheads',
  53: 'Headless arrow',
  882: 'Bronze arrow',
  884: 'Iron arrow',
  886: 'Steel arrow',
  888: 'Mithril arrow',
  890: 'Adamant arrow',
  892: 'Rune arrow',
  11212: 'Dragon arrow',
  11237: 'Dragon arrowheads',
  // Fletching - Logs
  1511: 'Logs',
  1513: 'Magic logs',
  1515: 'Yew logs',
  1517: 'Maple logs',
  1519: 'Willow logs',
  1521: 'Oak logs',
  // Fletching - Unstrung bows
  48: 'Longbow (u)',
  50: 'Shortbow (u)',
  54: 'Oak shortbow (u)',
  56: 'Oak longbow (u)',
  58: 'Willow longbow (u)',
  60: 'Willow shortbow (u)',
  62: 'Maple longbow (u)',
  64: 'Maple shortbow (u)',
  66: 'Yew longbow (u)',
  68: 'Yew shortbow (u)',
  70: 'Magic longbow (u)',
  72: 'Magic shortbow (u)',
  // Fletching - Strung bows
  839: 'Longbow',
  841: 'Shortbow',
  843: 'Oak shortbow',
  845: 'Oak longbow',
  847: 'Willow longbow',
  849: 'Willow shortbow',
  851: 'Maple longbow',
  853: 'Maple shortbow',
  855: 'Yew longbow',
  857: 'Yew shortbow',
  859: 'Magic longbow',
  861: 'Magic shortbow',
  1777: 'Bowstring',
  // Herblore - Grimy herbs
  199: 'Grimy guam leaf',
  201: 'Grimy marrentill',
  203: 'Grimy tarromin',
  205: 'Grimy harralander',
  207: 'Grimy ranarr weed',
  209: 'Grimy irit leaf',
  211: 'Grimy avantoe',
  213: 'Grimy kwuarm',
  215: 'Grimy cadantine',
  217: 'Grimy dwarf weed',
  219: 'Grimy torstol',
  2485: 'Grimy lantadyme',
  3049: 'Grimy toadflax',
  3051: 'Grimy snapdragon',
  // Herblore - Clean herbs
  249: 'Guam leaf',
  251: 'Marrentill',
  253: 'Tarromin',
  255: 'Harralander',
  257: 'Ranarr weed',
  259: 'Irit leaf',
  261: 'Avantoe',
  263: 'Kwuarm',
  265: 'Cadantine',
  267: 'Dwarf weed',
  269: 'Torstol',
  2481: 'Lantadyme',
  2998: 'Toadflax',
  3000: 'Snapdragon',
  // Herblore - Vial & secondaries
  227: 'Vial of water',
  221: 'Eye of newt',
  223: "Red spiders' eggs",
  225: 'Limpwurt root',
  231: 'Snape grass',
  235: 'Unicorn horn dust',
  239: 'White berries',
  245: 'Wine of zamorak',
  1975: 'Chocolate dust',
  2970: 'Mort myre fungus',
  3138: 'Potato cactus',
  6693: 'Crushed nest',
  // Herblore - Unfinished potions
  91: 'Guam potion (unf)',
  93: 'Marrentill potion (unf)',
  95: 'Tarromin potion (unf)',
  97: 'Harralander potion (unf)',
  99: 'Ranarr potion (unf)',
  101: 'Irit potion (unf)',
  103: 'Avantoe potion (unf)',
  105: 'Kwuarm potion (unf)',
  107: 'Cadantine potion (unf)',
  109: 'Dwarf weed potion (unf)',
  111: 'Torstol potion (unf)',
  2483: 'Lantadyme potion (unf)',
  3002: 'Toadflax potion (unf)',
  3004: 'Snapdragon potion (unf)',
  // Herblore - Finished potions
  115: 'Strength potion(3)',
  121: 'Attack potion(3)',
  127: 'Restore potion(3)',
  133: 'Defence potion(3)',
  139: 'Prayer potion(3)',
  145: 'Super attack(3)',
  157: 'Super strength(3)',
  163: 'Super defence(3)',
  169: 'Ranging potion(3)',
  175: 'Antipoison(3)',
  181: 'Superantipoison(3)',
  3010: 'Energy potion(3)',
  3018: 'Super energy(3)',
  3026: 'Super restore(3)',
  3042: 'Magic potion(3)',
  6687: 'Saradomin brew(3)',
  // Smithing - Ores
  436: 'Copper ore',
  438: 'Tin ore',
  440: 'Iron ore',
  442: 'Silver ore',
  444: 'Gold ore',
  447: 'Mithril ore',
  449: 'Adamantite ore',
  451: 'Runite ore',
  453: 'Coal',
  // Smithing - Bars
  2349: 'Bronze bar',
  2351: 'Iron bar',
  2353: 'Steel bar',
  2359: 'Mithril bar',
  2361: 'Adamantite bar',
  2363: 'Runite bar',
  // Smithing - Tools & items
  2: 'Cannonball',
  4: 'Ammo mould',
  2347: 'Hammer',
  1115: 'Iron platebody',
  1117: 'Bronze platebody',
  1119: 'Steel platebody',
  1121: 'Mithril platebody',
  1123: 'Adamant platebody',
  863: 'Iron knife',
  865: 'Steel knife',
  866: 'Mithril knife',
  867: 'Adamant knife',
  868: 'Rune knife',
  // Cooking - Raw fish
  317: 'Raw shrimps',
  327: 'Raw sardine',
  331: 'Raw salmon',
  335: 'Raw trout',
  341: 'Raw cod',
  345: 'Raw herring',
  349: 'Raw pike',
  353: 'Raw mackerel',
  359: 'Raw tuna',
  371: 'Raw swordfish',
  377: 'Raw lobster',
  383: 'Raw shark',
  389: 'Raw manta ray',
  395: 'Raw sea turtle',
  2138: 'Raw chicken',
  3142: 'Raw karambwan',
  7944: 'Raw monkfish',
  11934: 'Raw dark crab',
  13439: 'Raw anglerfish',
  // Cooking - Cooked food
  315: 'Shrimps',
  325: 'Sardine',
  329: 'Salmon',
  333: 'Trout',
  339: 'Cod',
  347: 'Herring',
  351: 'Pike',
  355: 'Mackerel',
  361: 'Tuna',
  373: 'Swordfish',
  379: 'Lobster',
  385: 'Shark',
  391: 'Manta ray',
  397: 'Sea turtle',
  2140: 'Cooked chicken',
  3144: 'Cooked karambwan',
  7946: 'Monkfish',
  11936: 'Dark crab',
  13441: 'Anglerfish',
  // Magic - Superheat/Plank Make/Tan Leather
  960: 'Plank',
  8778: 'Oak plank',
  8780: 'Teak plank',
  8782: 'Mahogany plank',
  6332: 'Mahogany logs',
  6333: 'Teak logs',
  1745: 'Green dragon leather',
  2505: 'Blue dragon leather',
  2507: 'Red dragon leather',
  2509: 'Black dragon leather',
  1747: 'Black dragonhide',
  1749: 'Red dragonhide',
  1751: 'Blue dragonhide',
  1753: 'Green dragonhide',
  // Spin Flax / Superglass Make
  1779: 'Flax',
  1783: 'Bucket of sand',
  1775: 'Molten glass',
  401: 'Seaweed',
  21504: 'Giant seaweed',
  // Gold amulet (strung)
  1692: 'Gold amulet',
  // Grimy/Clean herbs (for Degrime - already in herb sections)
  // Runecraft
  559: 'Body rune',
  560: 'Death rune',
  562: 'Chaos rune',
  7936: 'Pure essence',
  21880: 'Wrath rune',
};

export function calculateMethodCost(method: Method, prices: PriceMap): { costPerAction: number, inputCosts: { name: string; id: number; price: number; count: number; total: number }[] } {
  let costPerAction = 0;
  const inputCosts = method.inputs.map((input) => {
    const priceData = prices[input.id];
    // Use 'avgHighPrice' (24h buy avg) if available, else 'high' (latest buy instabuy)
    const price = priceData?.avgHighPrice ?? priceData?.high ?? 0;
    const total = price * input.count;
    costPerAction += total;
    return {
      name: ITEM_NAMES[input.id] || `Item ${input.id}`,
      id: input.id,
      price,
      count: input.count,
      total,
    };
  });

  return { costPerAction, inputCosts };
}

export function evaluateMethod(
  method: Method,
  prices: PriceMap,
  playerStats: Record<Skill, number>,
  completedQuests?: Set<string>
): MethodEvaluation {
  // 1. Check Requirements
  const missingRequirements: Requirement[] = [];
  let isLocked = false;

  for (const req of method.requirements) {
    const playerLevel = playerStats[req.skill] || 0;
    if (playerLevel < req.level) {
      missingRequirements.push(req);
      isLocked = true;
    }
  }

  // 1b. Check Quest Requirements
  const missingQuests: string[] = [];
  if (method.quests) {
    for (const quest of method.quests) {
      if (!completedQuests || !completedQuests.has(quest)) {
        missingQuests.push(quest);
        isLocked = true;
      }
    }
  }

  // 2. Calculate Costs
  const { costPerAction, inputCosts } = calculateMethodCost(method, prices);

  // 3. Calculate Revenue
  let revenuePerAction = 0;
  const outputRevenues = method.outputs.map((output) => {
    const priceData = prices[output.id];
    // Use 'avgLowPrice' (24h sell avg) if available, else 'low' (latest sell instasell)
    const price = priceData?.avgLowPrice ?? priceData?.low ?? 0;
    const total = price * output.count;
    revenuePerAction += total;
    const volume = (priceData?.highPriceVolume ?? 0) + (priceData?.lowPriceVolume ?? 0);
    return {
      name: ITEM_NAMES[output.id] || `Item ${output.id}`,
      id: output.id,
      price,
      count: output.count,
      total,
      volume,
    };
  });

  // 4. Profit
  const profitPerAction = revenuePerAction - costPerAction;
  const effectiveTicksPerAction = method.ticksPerAction + (method.bankTimePerAction ?? 0);
  const actionsPerHour = 6000 / effectiveTicksPerAction;
  const profitPerHour = profitPerAction * actionsPerHour;

  const expPerHour = method.exp.map(exp => ({
    skill: exp.skill,
    amount: Math.floor(exp.amount * actionsPerHour),
  }));
  const totalExpPerHour = expPerHour.reduce((sum, e) => sum + e.amount, 0);

  return {
    methodId: method.id,
    methodName: method.name,
    isLocked,
    missingRequirements,
    missingQuests,
    costPerAction,
    revenuePerAction,
    profitPerAction,
    profitPerHour,
    inputCosts,
    outputRevenues,
    expPerHour,
    totalExpPerHour,
    actionsPerHour,
  };
}
